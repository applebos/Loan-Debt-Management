        ...
        <div className="mb-8 p-6 border border-gray-200 rounded-lg bg-gray-50">
            <div className="flex border-b mb-4">
                <button 
                    ...
                    className={`px-4 py-2 text-base font-semibold transition-colors duration-200 ${activeTab === 'description' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-blue-600'}`}>
                    설명
                </button>
                <button 
                    onClick={() => setActiveTab('methods')} 
                    className={`px-4 py-2 text-base font-semibold transition-colors duration-200 ${activeTab === 'methods' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-blue-600'}`}>
                    상환 방법
                </button>
            </div>
            <div className="pt-2 text-sm text-gray-800 min-h-[120px]">
                {activeTab === 'methods' ? (
        ...
        <form action={formAction} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div className="flex items-center">
                    <label htmlFor="principal" className={labelStyles}>대출 금액</label>
                    <input type="number" id="principal" name="principal" required className={inputStyles} placeholder="금액 입력" />
                    <span className={addonStyles}>만원</span>
                </div>
                <div className="flex items-center">
                    <label htmlFor="annualRate" className={labelStyles}>연 이자율</label>
                    <input type="number" step="0.01" id="annualRate" name="annualRate" required className={inputStyles} placeholder="% 입력" />
                    <span className={addonStyles}>%</span>
                </div>
                <div className="flex items-center">
                  <label htmlFor="loanTermMonths" className={labelStyles}>대출 기간</label>
                  <input type="number" id="loanTermMonths" name="loanTermMonths" required className={inputStyles} placeholder="총 개월수" />
                  <span className={addonStyles}>개월</span>
                </div>
                {showGracePeriod && (
                  <div className="flex items-center">
                    <label htmlFor="gracePeriodMonths" className={labelStyles}>거치 기간</label>
                    <input type="number" id="gracePeriodMonths" name="gracePeriodMonths" className={inputStyles} placeholder="거치 개월수" />
                    <span className={addonStyles}>개월</span>
                  </div>
                )}
            </div>
            ...
            {showPrepayment && additionalRepayments.map((_, index) => (
                 <div key={index} className="p-3 bg-gray-50 rounded-lg">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                         <div className="flex items-center">
                            <label className="shrink-0 text-gray-700 text-sm w-16 text-center">회차</label>
                            <input type="number" name={`repaymentRound_${index}`} placeholder="상환 회차" className={`${inputStyles} text-sm`} />
                            <span className={`${addonStyles} text-sm`}>회</span>
                        </div>
                        <div className="flex items-center">
                            <label className="shrink-0 text-gray-700 text-sm w-16 text-center">상환액</label>
                            <input type="number" name={`repaymentAmount_${index}`} placeholder="금액" className={`${inputStyles} text-sm`} />
                            <span className={`${addonStyles} text-sm`}>만원</span>
                        </div>
                        <div className="flex items-center">
                            <label className="shrink-0 text-gray-700 text-sm w-16 text-center">변경 이율</label>
                            <input type="number" step="0.01" name={`newRate_${index}`} placeholder="변경 시만" className={`${inputStyles} text-sm`} />
                            <span className={`${addonStyles} text-sm`}>%</span>
                        </div>
                    </div>
                     <div className="flex justify-end items-center mt-2">
                        {additionalRepayments.length > 1 && (
                            <button type="button" onClick={() => removeRepayment(index)} className="w-7 h-7 flex items-center justify-center bg-red-500 text-white rounded-full text-lg shrink-0 hover:bg-red-600 transform hover:scale-110 transition-transform">-</button>
                        )}
                        {index === additionalRepayments.length - 1 && (
                            <button type="button" onClick={addRepayment} className="ml-2 w-7 h-7 flex items-center justify-center bg-blue-500 text-white rounded-full text-lg shrink-0 hover:bg-blue-600 transform hover:scale-110 transition-transform">+</button>
                        )}
                    </div>
                </div>
            ))}
...
const RefinanceSuggestion = ({ refinanceInfo, annualRate }: { refinanceInfo: RefinanceInfo, annualRate: number }) => {
    ...
    const externalLinks = [
        ...
        { name: '토스', url: 'https://toss.im/credit-loan/refinance' },
        { name: '네이버페이', url: 'https://new-m.pay.naver.com/loans/refinance' },
        { name: '핀다', url: 'https://www.finda.co.kr/loan-replacement' },
    ];
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4 font-sans">
            <div className="w-full max-w-5xl mx-auto bg-white rounded-2xl shadow-xl p-8">
                ...
                {state.results && <ResultsDisplay results={state.results} visibleSchedule={visibleSchedule} handleToggleSchedule={handleToggleSchedule} />}
